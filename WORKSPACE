...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# ADD BACK rules_shell – needed by org_tensorflow (TFLite sh_test)
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
# Load Zlib before initializing TensorFlow to guarantee @zlib//:mini_zlib exists
http_archive(
    name = "zlib",
...
# ───────── Java / Maven deps (Android artifacts kept but harmless on Linux) ─────────
...
# org_tensorflow depends on XNNPACK
http_archive(
    name = "XNNPACK",
...
# ───────── Rules_pkg / rules_python (for python deps) ─────────
...
# ───────── rules_foreign_cc / bazel_features ─────────
...
# ───────── rules_ml_toolchain (CUDA toolchain) ─────────
...
# ───────── Misc third_party (external_files / wasm) ─────────
...
# ───────── Halide (Linux only – macOS/Windows removed) ─────────
...
# Description for foreign_cc-based CMake packages
all_content = """filegroup(name = "all", srcs = glob(["**"]), visibility = ["//visibility:public"])"""

# Google Benchmark library v1.6.1
http_archive(
...
# protobuf-javalite via source download
http_archive(
